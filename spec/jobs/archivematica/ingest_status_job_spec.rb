RSpec.describe Archivematica::IngestStatusJob do
  ...
  describe 'successful job' do
    ...
    context 'COMPLETE' do
      ...
      it 'performs the job, queues the next job and calls the job status service' do
        expect(Archivematica::PackageDetailsJob).to receive(:set).with(wait: 5.minutes)
        expect(Archivematica::PackageDetailsJob).to receive(:perform_later)
    ...
    context 'ERROR' do
      ...
      it 'performs the job and calls the job status service' do
        expect(JobStatusService).to receive(:new).with(hash_including(:job_status_id, :job, job_status_id: 'job_status_id', status: 'error', message: '200: ERROR'))
        described_class.perform_now(
          job_status_id: 'job_status_id',
          uuid: 'uuid'
        )
  ...
  describe 'unsuccessful job, do not retry' do
    ...
    it 'performs the job, queues the next job and calls the job status service' do
      expect(Archivematica::PackageDetailsJob).not_to receive(:perform_later)
